...
/* hide auto-generated sidebar footer & buttons */
.btn, .site-footer, .project-tagline{ display:none!important; }

/* uniform gold colour for links and <summary> toggles */
...
<div id="innotech-responsibilities">
    <ul>
        ...
        <li class="hidden">Написание инструкций для разработчиков по алгоритмам работы с внутренними фреймворками (xml).</li>
        <li class="hidden">Создание и регистрация .jar (Scala, Spark, Maven) модулей для загрузки больших (~100-150 Gb) таблиц в staging слой DWH команды.</li>
        <li class="hidden">Поставка всех выполненных работ до production среды в рамках CI/CD процесса (git, sfera, Liquibase, YAML).</li>
    </ul>
    <a href="javascript:void(0);" onclick="toggleInnotech()">Show more</a>
</div>
...
<script>
function toggleInnotech() {
    var list = document.getElementById('innotech-responsibilities');
    var hiddenItems = list.querySelectorAll('.hidden');
    var link = list.querySelector('a');

    for (var i = 0; i < hiddenItems.length; i++) {
        hiddenItems[i].style.display = (hiddenItems[i].style.display === 'none') ? 'list-item' : 'none';
    }
